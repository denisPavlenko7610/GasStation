...
{
    /// <summary>Copies simulation state into HudModel for the MonoBehaviour UI.</summary>
    ...
    public partial class HudBridgeSystem : SystemBase
    {
        protected override void OnUpdate()
        {
            ...
            {
                if (car.ValueRO.State == CarState.Queued || car.ValueRO.State == CarState.Arriving)
                    queueLength++;
                if (car.ValueRO.State != CarState.Leaving)
                    carsOnSite++;
            }
        ...
        {
            var info = new PumpInfo { Number = pump.Number };
